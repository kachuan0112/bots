...
class Music(commands.Cog):
    ...
    async def on_ready(self):
        print('Music_cog Loaded Succesfully')
...
class Song:
    ...
    def create_embed(self):
        embed = embed = (discord.Embed(title='<:YouTube:903586158859132959> Now Playing :notes:', description='**```py\n{0.source.title}\n```**'.format(self), color=discord.Color.blurple())
    ...
    async def audio_player_task(self):
        while True:
            ...
            if self.loop == False:
                ...
                try:
                    async with timeout(7200):  # 2 hours
...
class Music(commands.Cog):
    ...
    @commands.Cog.listener()
    async def on_ready(self):
      print('Music_cog listener is ready.')
    ...
    async def _queue(self, ctx: commands.Context, *, page: int = 1):
        ...
        if len(ctx.voice_state.songs) == 0:
            await ctx.send('Current No More Music Playing')
    ...
    async def _play(self, ctx: commands.Context, *, search: str):
        async with ctx.typing():
            try:
                ...
                await ctx.send('An error occurblurple while processing this request: {}'.format(str(e)))
            else:
                if not ctx.voice_state.voice:
